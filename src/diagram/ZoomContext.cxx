...
ZoomContext::on_button_press_event(Shape* shape, GdkEventButton* e)
{
    if (e->button == Left_Button || e->button == Right_Button) {
        m_active = true;
        m_moved  = false;
        m_mpoint = m_opoint = m_display->zwindow()->to_real_coord(point_of_event(e));
    }
...
ZoomContext::on_motion_notify_event(Shape* shape, GdkEventMotion* e)
{
    ...
    Point current = m_display->zwindow()->to_real_coord(point_of_event(e));
    m_moved = true;
...
ZoomContext::on_button_release_event(Shape* shape, GdkEventButton* e)
{
    ...
    if (m_moved) {
        Point origin(min(real_opoint.x, real_current.x),
                     min(real_opoint.y, real_current.y));
        ...
        m_display->zoom(origin, factor, Display::ZoomPoint_Origin);
    } else {
        factor = larger ? 2.0 : 0.5;
        m_display->zoom(real_current, factor, Display::ZoomPoint_Center);
    }
